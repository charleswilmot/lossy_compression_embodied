...
class SimulationConsumer(SimulationConsumerAbstract):
    ...
    def add_arm(self, position=None, orientation=None, from_tech_sheet=False):
        ...
        model = Kuka(model.get_handle(), count=len(self._shapes['arm']))
        if position is not None:
            model.set_position(position)
        if orientation is not None:
            model.set_orientation(orientation)
    ...
    @communicate_return_value
    def add_camera(self, position=None, orientation=None, resolution=[320, 240], view_angle=60.0):
    ...
    def get_tips(self):
        ...
        for arm in self._arm_list:
            next += 3
            self._arm_tips_buffer[last:next] = arm.get_tip().get_position()
            last = next
        return self._arm_tips_buffer
    ...
    def set_joint_positions(self, positions):
        ...
        for arm, joint_count in zip(self._arm_list, self._arm_joints_count):
            next += joint_count
            arm.set_joint_positions(positions[last:next])
            last = next
    ...
    def get_joint_intervals(self):
        ...
        for arm, joint_count in zip(self._arm_list, self._arm_joints_count):
            next += joint_count
            is_cyclic, intervals = arm.get_joint_intervals()
            self._intervals[last:next] = intervals
            last = next
